...
3. Use `names()` to name the `prices` using `countries`. Show the first 5 values of the named vector of prices.

4. Use `round()` to round the prices to the 3rd decimal. Overwrite `prices` with the rounded prices.

5. Use name indexing to report the prices for Canada, United States, Mexico
...
8. In your narrative, mention that we are going to drop the 46th element. Use inline code to report the country that will drop.

9. Overwrite the vector of prices with a new vector without observation 46. Use `length()` to show that you have one observation less.
